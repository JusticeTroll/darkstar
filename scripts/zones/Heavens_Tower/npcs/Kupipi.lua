...
function onTrigger(player,npc)
    ...
    if (Level >= 5 and TrustWindurst == QUEST_ACCEPTED and (TrustBastok == QUEST_COMPLETED or TrustSandoria == QUEST_COMPLETED)) then
        if (player:hasKeyItem(dsp.ki.GREEN_INSTITUTE_CARD) == true and player:hasSpell(898) == false) then
    ...
    elseif (Level >= 5 and TrustWindurst == QUEST_ACCEPTED) then
        ...
        elseif (player:hasSpell(898) == true and player:getVar("WindurstFirstTrust") == 1) then
    ...
    elseif(TrustWindurst == QUEST_COMPLETED and player:hasSpell(901) == false) then
        player:startEvent(438,0,0,0,0,0,0,0,rank3); --TRUST
    else
        player:startEvent(251);
    end
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 437) then
        ...
        player:delKeyItem(dsp.ki.GREEN_INSTITUTE_CARD);
