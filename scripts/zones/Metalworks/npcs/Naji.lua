...
function onTrigger(player,npc)
    ...
    if (Level >= 5 and TrustBastok == QUEST_ACCEPTED and (TrustSandoria == QUEST_COMPLETED or TrustWindurst == QUEST_COMPLETED)) then
        if (player:hasKeyItem(dsp.ki.BLUE_INSTITUTE_CARD) == true and player:hasSpell(897) == false) then
    ...
    elseif (Level >= 5 and TrustBastok == QUEST_ACCEPTED) then
        ...
        elseif (player:hasSpell(897) == true and player:getVar("BastokFirstTrust") == 1) then
    ...
    --TRUST END    
    elseif(TrustBastok == QUEST_COMPLETED and player:hasSpell(900) == false) then
...
function onEventFinish(player,csid,option)
    ...
    elseif (csid == 982) then
        ...
        player:delKeyItem(dsp.ki.BLUE_INSTITUTE_CARD);
