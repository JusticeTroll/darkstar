...
inline int32 CLuaBaseEntity::PrintToArea(lua_State* L)
{
    ...
    else if (messageRange == 3) // Party and Alliance
    {
        if (PChar->PParty != nullptr)
        {
            ...
            {
                member->pushPacket(new CChatMessagePacket(member, messageLook, (char*)lua_tostring(L, 1), name));
            }
            //if ()
            //{
            //    PChar->pushPacket(new CChatMessagePacket(PChar, messageLook, message, name));
            //}
            //int8 packetData[8]{};
            //ref<uint32>(packetData, 0) = PChar->PParty->GetPartyID();
            //ref<uint32>(packetData, 4) = PChar->id;
            //message::send(MSG_CHAT_PARTY, 0, 0, new CChatMessagePacket(PChar, messageLook, message, name));
            //message::send(MSG_CHAT_PARTY, packetData, sizeof packetData, new CChatMessagePacket(PChar, messageLook,  (char*)lua_tostring(L, 1), name));
        }
    }
...
inline int32 CLuaBaseEntity::getRank(lua_State *L)
{
    ...
    if (lua_isnil(L, 1) || (!lua_isnumber(L, 1) && !lua_istable(L, 1)))
    {
    ...
    else
    {
        ...
        uint8 nation = (uint8)lua_tointeger(L, lua_isnumber(L, 1) ? 1 : -1);

        lua_pushinteger(L, PChar->profile.rank[nation]);
    }
    return 1;
}
...
*  Example : player:setRank(BASTOK, 10)
*  Notes   :
************************************************************************/
...
inline int32 CLuaBaseEntity::setRank(lua_State *L)
{
    ...
    PChar->profile.rank[PChar->profile.nation] = (int32)lua_tointeger(L, 2);;
...
inline int32 CLuaBaseEntity::takeDamage(lua_State *L)
{
    ...
    if (damage > 0) {
        ...
        if (!lua_isnil(L, 2) && lua_isuserdata(L, 2)) {
            ...
            DSP_DEBUG_BREAK_IF(PLuaAttacker == nullptr);
...
/************************************************************************
*  Function: capAllTrustSkills()
*  Purpose : Cap all skills for a Trust
*  Example : player:capAllTrustSkills()
*  Notes   :
************************************************************************/

inline int32 CLuaBaseEntity::capAllTrustSkills(lua_State* L)
{
    DSP_DEBUG_BREAK_IF(m_PBaseEntity == nullptr);
...
*  Example : player:addLatent(dsp.latent.LATENT_HP_UNDER_PERCENT, 95, dsp.mod.REGEN, 1)
*  Notes   :
************************************************************************/
...
*  Example : player:delLatent(dsp.latent.LATENT_HP_UNDER_PERCENT, 95, dsp.mod.REGEN, 1)
*  Notes   :
************************************************************************/
...
*  Example : if (attacker:getWeaponDamageType(SLOT_MAIN) == dsp.damageType.PIERCING)
...
int32 CLuaBaseEntity::takeWeaponskillDamage(lua_State* L)
{
    ...
    DSP_DEBUG_BREAK_IF(lua_isnil(L, 4) || !lua_isnumber(L, 4));
    DSP_DEBUG_BREAK_IF(lua_isnil(L, 5) || !lua_isnumber(L, 5));
    DSP_DEBUG_BREAK_IF(lua_isnil(L, 6) || !lua_isboolean(L, 6));
    DSP_DEBUG_BREAK_IF(lua_isnil(L, 7) || !lua_isnumber(L, 7));
    DSP_DEBUG_BREAK_IF(lua_isnil(L, 8) || !lua_isnumber(L, 8));
    DSP_DEBUG_BREAK_IF(lua_isnil(L, 9) || !lua_isnumber(L, 9));
...
inline int32 CLuaBaseEntity::useTrustAbility(lua_State* L)
{
    ...
    if (lua_isnumber(L, 1))
    {
        ...
        if (!lua_isnil(L, 2) && lua_isuserdata(L, 2))
        {
            ...
            trust->PAI->Ability(PTarget->targid, (uint16)lua_tointeger(L, 1));
        }
        else
        {
            trust->PAI->Ability(trust->targid, (uint16)lua_tointeger(L, 1));
        }
    }
...
inline int32 CLuaBaseEntity::getPartyTrusts(lua_State* L)
{
    ...
    lua_createtable(L, master->PTrusts.size(), 0);

    if (master->PTrusts.size() > 0)
    {
        int i = 1;
...
Lunar<CLuaBaseEntity>::Register_t CLuaBaseEntity::methods[] =
{
    ...
    LUNAR_DECLARE_METHOD(CLuaBaseEntity,capAllTrustSkills),    LUNAR_DECLARE_METHOD(CLuaBaseEntity,capAllSkills),
