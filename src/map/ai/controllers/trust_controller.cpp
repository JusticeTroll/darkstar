...
bool CTrustController::IsWeaponSkillReady(float currentDistance)
{
    ...
    time_point time = m_LastWeaponSkillTime + std::chrono::seconds(PTrust->getMobMod(MOBMOD_SPECIAL_COOL));
    return (m_Tick >= time);
...
bool CTrustController::IsSpellReady(float currentDistance)
{
    ...
    time_point time = m_LastMagicTime + std::chrono::seconds(PTrust->getMobMod(MOBMOD_MAGIC_COOL) - bonusTime);
