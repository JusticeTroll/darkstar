...
uint8 CParty::GetMemberPartyPosition(uint32 MemberId)
{
    for (uint32 i = 0; i < members.size(); ++i)
    {
        if (members.at(i)->id == MemberId)
        {
            return i;
        }
    }
...
CBattleEntity* CParty::GetMemberFromPosition(uint8 PPosition)
{
    return members.at(PPosition);
}
